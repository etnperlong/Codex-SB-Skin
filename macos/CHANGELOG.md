...
- schema v2 的超宽横幅在 Home 保持为 token 控制的 Hero 卡片，避免浅色素材被放大成暗色壳层的整窗背景并降低文字对比度
- 浅色侧栏覆盖原生 Tailwind 重要级占位文字规则，“展开显示”等辅助操作不再变成半透明白字
- Home 建议卡图标改为显式双轴 flex 居中，不再被上游 `justify-start` 工具类推到图标底板左侧
