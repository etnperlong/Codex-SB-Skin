...
- 内置主题更新为 `Small & Beauty`（小而美）：使用克制的灰白层级、微信绿状态语义、紧凑横向建议卡和抽象对话横幅
- `theme.json` 支持语义化 schema v2：`tokens` 提供浅色 / 深色两套完整视觉 token（侧栏、主界面、消息、输入框、卡片、项目选择器、弹层、状态色、排版、圆角、布局、阴影、模糊和动效）
- 运行时把 271 个语义 token 映射为 `--ds-<group>-<token>` CSS 自定义属性，并在浅色 / 深色壳切换时自动更新
- v2 主题的十个核心色彩角色（canvas、surface、accent、text 等）会同步驱动旧 `--ds-*` 变量及其派生 `-rgb` 通道，与自适应图像引擎、`appearance` 和 `art.*` 构图控制保持兼容

### 兼容

- schema v1 主题继续原样加载与渲染，不会被改写；语义样式层仅对声明 `schemaVersion: 2` 的主题激活
- 主题 token 使用严格 CSS 值校验，拒绝包含声明注入字符的不安全配置

### 修复

- schema v2 超宽素材在 Home 保持为 token 控制的 Hero，不会覆盖整窗并降低浅色素材与暗色文字的对比度
- 侧栏重要级工具类、Tooltip 子节点和建议卡图标均显式继承语义 token，适配新版 Codex DOM 与 Tailwind 级联
- 横向建议卡会清除原生文案容器残留的底部对齐约束，使图标、文字和卡片中心保持在同一水平线上；实时验证同步检查该几何关系
- 浅色侧栏的 muted 色提高到可读对比度，“展开显示”等辅助操作不再变成半透明白字
